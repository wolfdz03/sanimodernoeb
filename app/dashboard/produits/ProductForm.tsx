...
import { X, Plus, ChevronUp, Check, Upload, Trash2 } from "lucide-react";
import {
...
export function ProductForm({
  ...
  function removeOption(optionIndex: number) {
    const option = optionTypes[optionIndex];
    if (!option) return;
    const affectedCount = variants.filter((variant) =>
      variant.options?.some((item) => item.typeName === option.name)
    ).length;
    if (
      affectedCount > 0 &&
      !confirm(`Supprimer l'option « ${option.name} » supprimera aussi ${affectedCount} variante${affectedCount > 1 ? "s" : ""}. Continuer ?`)
    ) return;
    ...
    setVariants((prev) =>
      prev.filter((variant) => !variant.options?.some((item) => item.typeName === option.name))
    );
    setNewOptionValueByIndex({});
  }
  ...
  function removeOptionValue(optionIndex: number, valueIndex: number) {
    ...
    const affectedCount = variants.filter((variant) =>
      variant.options?.some(
        (item) => item.typeName === option.name && item.value === optionValue.value
      )
    ).length;
    if (
      affectedCount > 0 &&
      !confirm(`Supprimer « ${optionValue.value} » supprimera aussi ${affectedCount} variante${affectedCount > 1 ? "s" : ""}. Continuer ?`)
    ) return;
        ...
        index === optionIndex
          ? {
              ...
              values: (type.values ?? [])
                ...
                .map((value, currentValueIndex) => ({ ...value, sort_order: currentValueIndex })),
            }
          : type
      )
    );
  ...
  function clearVariants() {
    if (variants.length > 0 && !confirm(`Supprimer les ${variants.length} variantes ?`)) return;
    setVariants([]);
  }
            ...
            <div className={activeTab === "variants" ? "space-y-6" : "hidden"}>
                      ...
                      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                        <div className="md:col-span-1">
                          ...
                          <div className="flex gap-2">
                            ...
                            <button
                              type="button"
                              onClick={() => removeOption(ti)}
                              className="flex min-h-11 min-w-11 shrink-0 items-center justify-center rounded-lg border border-red-100 text-red-600 hover:bg-red-50"
                              aria-label={`Supprimer l'option ${ot.name}`}
                          ...
                          <div className="flex flex-wrap items-center gap-2 p-1.5 w-full rounded-md border border-[var(--dash-border)] bg-[var(--dash-surface)] focus-within:ring-1 focus-within:ring-[var(--dash-primary)] focus-within:border-[var(--dash-primary)] transition-all min-h-[42px]">
                            {(ot.values ?? []).map((val, vi) => (
                              ...
                              >
                                ...
                                <button
                                  type="button"
                                  onClick={() => removeOptionValue(ti, vi)}
                                  className="ml-1 inline-flex h-7 w-7 items-center justify-center rounded-md text-[var(--dash-primary)]/60 hover:bg-red-100 hover:text-[var(--dash-primary)] focus:outline-none focus-visible:ring-2 focus-visible:ring-[var(--dash-primary)]"
                ...
                <div>
                    ...
                    <div className="flex items-center gap-2">
                      ...
                      {variants.length > 0 && (
                        <button
                          ...
                          className="inline-flex min-h-10 items-center gap-1.5 rounded-lg border border-red-100 px-3 text-xs font-semibold text-red-600 hover:bg-red-50"
                        >
                  ...
                  <p className="mb-3 text-xs text-[var(--dash-text-muted)]">
                    Les suppressions sont appliquées définitivement après avoir cliqué sur Publier.
                  </p>
                    ...
                    <table className="min-w-[640px] w-full divide-y divide-[var(--dash-border)]">
                      ...
                      <tbody className="bg-[var(--dash-surface)] divide-y divide-[var(--dash-border)]">
                        {variants.length === 0 ? (
                          ...
                          variants.map((v, vi) => (
                            <tr key={vi}>
                              <td className="sticky left-0 z-[1] bg-white px-3 py-2 text-sm font-medium text-[var(--dash-text-main)] sm:px-6 sm:py-4">
                                <div className="flex min-w-[170px] items-center justify-between gap-3">
                                  <span className="whitespace-nowrap">{formatVariantLabel(v.options)}</span>
                                  <button
                                    type="button"
                                    onClick={() => removeVariant(vi)}
                                    className="flex min-h-11 min-w-11 shrink-0 items-center justify-center rounded-lg border border-red-100 text-red-600 hover:bg-red-50"
                                    aria-label={`Supprimer la variante ${formatVariantLabel(v.options)}`}
