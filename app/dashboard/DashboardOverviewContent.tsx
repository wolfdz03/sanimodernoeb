...
const statusStyles: Record<OrderStatus, string> = {
  pending:
    "text-orange-800 bg-orange-100 dark:bg-orange-900/30 dark:text-orange-300",
  paid: "text-blue-800 bg-blue-100 dark:bg-blue-900/30 dark:text-blue-300",
  shipped:
    "text-emerald-800 bg-emerald-100 dark:bg-emerald-900/30 dark:text-emerald-300",
  delivered:
    "text-emerald-800 bg-emerald-100 dark:bg-emerald-900/30 dark:text-emerald-300",
  cancelled: "text-red-800 bg-red-100 dark:bg-red-900/30 dark:text-red-300",
};
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
        <div className="bg-white dark:bg-[#0d1b1b] p-6 rounded-xl border border-slate-100 dark:border-slate-800 shadow-sm hover:shadow-md transition-shadow">
          ...
          <div className="mt-4 flex items-center gap-2">
            <span className="text-slate-600 dark:text-slate-400 text-xs">{t("dashboard_revenue_sub")}</span>
          </div>
        </div>
        ...
        >
          ...
          <div className="mt-4 flex items-center gap-2">
            <span className="text-slate-600 dark:text-slate-400 text-xs">{t("dashboard_needs_attention")}</span>
          </div>
        </Link>
        ...
        >
          ...
          <div className="mt-4 flex items-center gap-2">
            <span className="text-slate-600 dark:text-slate-400 text-xs">{t("dashboard_this_week")}</span>
          </div>
        </Link>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        <div className="lg:col-span-2 bg-white dark:bg-[#0d1b1b] rounded-xl border border-slate-100 dark:border-slate-800 shadow-sm overflow-hidden flex flex-col">
          <div className="p-6 border-b border-slate-100 dark:border-slate-800 flex items-center justify-between">
            ...
            <Link
              href="/dashboard/commandes"
              className="text-sm font-medium text-teal-700 dark:text-[#0ea5a5] hover:text-teal-800 dark:hover:text-[#13ecec] transition-colors"
            >
          ...
          <div className="overflow-x-auto">
            <table className="w-full text-start border-collapse">
              <thead>
                <tr className="text-xs font-semibold tracking-wide text-slate-600 dark:text-slate-400 uppercase border-b border-slate-100 dark:border-slate-800 bg-slate-50/50 dark:bg-slate-800/20">
                    ...
                    >
                      <td className="px-6 py-4 font-medium text-slate-900 dark:text-white">
                        <Link
                          ...
                          className="hover:text-teal-700 dark:hover:text-[#13ecec] transition-colors"
                        >
                          #{order.id.slice(0, 8)}…
                        </Link>
                      </td>
        ...
        <div className="bg-white dark:bg-[#0d1b1b] rounded-xl border border-slate-100 dark:border-slate-800 shadow-sm p-6 flex flex-col">
          ...
          <div className="flex-1 flex items-center justify-center relative my-4">
            {topCategories.length === 0 ? (
              <span className="text-slate-600 dark:text-slate-400 text-sm">{t("dashboard_no_categories")}</span>
            ) : (
              <div
